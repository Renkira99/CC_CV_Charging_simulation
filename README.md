...
## Section 9 — PCC Harmonic Analysis Module

### `pcc_harmonic_analysis.py`

This module computes actual harmonic currents in Amperes injected at the 440 V Point of Common Coupling (PCC) using the load profile generated by the simulator. It evaluates theoretical compliance limits and evaluates potential impacts on local distribution transformers.

1. **Fundamental and Harmonic Currents**: Calculates PCC fundamental current dynamically using simulated charging power and topology-derived power factor, superimposing harmonic components scaled accurately to normative literature (e.g., three-phase triplen harmonic cancellation).
2. **Standard Compliance Checking**: Checks measured current harmonics against IEC 61000-3-2 (Class A limits), IEC 61000-3-12 (percentage thresholds), or IEEE 519 / IS 16528 (Total Demand Distortion limit of 5%), correctly applying the limits based on the simulated charger rating and phase configuration.
3. **Transformer Derating**: Computes distribution transformer capacity derating under harmonic stress using the IEEE C57.110 harmonic loss factor (FHL). It evaluates capacity derating for a baseline 200 kVA transformer across 1, 3, and 5 simultaneous EV loads based on literature scaling multipliers.
4. **Output Plot**: Generates a 2×2 IEEE-style publication figure per simulated session (`fig4_pcc_harmonic_analysis.png`), providing a real-time plot of injected PCC currents, time-domain TDD curve, compliance limits via bar charts, and an effective FHL-based transformer derating analysis.
...
└── EV_Dynamic_Charging_Simulation_Results/
    ...
    │   └── fig4_pcc_harmonic_analysis.png
...
One subdirectory per charger preset, each containing four publication figures. Figure 3 (topology comparison) is identical across all preset subdirectories since it does not depend on the charging simulation output.
...
1. **OpenDSS feeder integration via `opendssdirect` Python library** — the `power_kw` time series from `simulate_charging()` will be passed directly as a `LoadShape` object to the feeder model without leaving Python for distribution network impact analysis.
